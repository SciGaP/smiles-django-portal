...
function getSMILESDataProduct(type, dp_id) {
    return axios
        .get(`http://127.0.0.1:8000/smiles/${type}-dp/${dp_id}`)
...
function getSMILESDataProducts(type, params) {
    return axios
        .get(`http://127.0.0.1:8000/smiles/${type}-dps?${params}`)
        .then((response) => Promise.resolve(response))
        .catch((error) => Promise.reject(error.response));
}

function deleteSMILESDataProduct(type, dp_id) {
    return axios.delete(`http://127.0.0.1:8000/smiles/${type}-dp/${dp_id}`)
}
